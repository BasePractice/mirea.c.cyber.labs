...
/*
 * Написать функцию, определяющую количество вхождений некоторой подстроки
 */


static int count_of(char *text, char *search)  {
    int ret = 0, i, equals = 0;
...
char *CALL(task)(struct Argument *args) {
    char buf[20];
    int ret = count_of(args->text[0], args->text[1]);
